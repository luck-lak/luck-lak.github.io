...
2. Read the latest relevant entries in `tools/agent/maintenance-log.md` for recent decisions and prior verification. The log is historical context, not proof of current state. Do not load `tools/agent/history/` by default; search it by task area or keyword only when older context is useful.
