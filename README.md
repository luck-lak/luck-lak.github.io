...
| 页面宽度、文字大小、深色模式、手机布局 | `css/` 功能模块，由 `css/style.css` 统一加载 |
| 主题切换和记忆 | `js/main.js` |
...
页面仍只引用 `css/style.css`。它是加载入口，按顺序导入以下模块，不放具体样式规则；无需额外构建工具。

| 文件（位于 `css/`） | 职责 |
| --- | --- |
| `base.css` | 基础排版、页眉导航、主题按钮和页脚 |
| `home.css` | 首页介绍、学习方向、项目、学习轨迹与联系区域 |
| `learning.css` | 学习总入口、平台列表、课程卡片和下载入口 |
| `records.css` | 笔记阅读布局、目录、正文、图片、表格和上下篇导航 |
| `chapter-cards.css` | 长笔记的章节卡片及目录目标强调 |
| `code-blocks.css` | 代码卡片、复制按钮和语法颜色 |
| `accessibility.css` | 键盘焦点和减少动画设置 |
| `blog.css` | 博客列表、封面和文章正文 |

修改现有功能时，优先在对应文件中添加或修改规则；该功能的深色模式与手机适配也放在同一文件。全站焦点和减少动画设置集中在 `accessibility.css`。

当新功能有独立职责、可复用组件，或现有模块开始混入明显不同的功能时，再拆出新的 CSS 文件，并在 `style.css` 添加导入及在此表登记。判断依据是职责与维护成本，不设固定行数门槛；不要为一次小改动创建零碎文件，也不要把规则追加回入口。

导入顺序影响覆盖：基础在前，页面与组件在后；章节卡片和代码卡片必须在 `records.css` 后。调整顺序或跨文件移动规则时，对比深浅主题及窄屏下的结果。当前所有页面共用一个入口，首次加载会多请求几个可缓存的小文件；以后如需按页加载，应同步修改普通 HTML、生成器和 Jekyll 头部模板。
